# code to train backward mapping regression from world coordinates
...
def write_log_file(experiment_name,losses, epoch, phase):
    with open('./checkpoints-bm-swat3d/'+experiment_name+'.txt','a') as f:
        f.write("\n{} Epoch: {} Loss: {} MSE: {} UnwarpL2: {} UnwarpSSIMloss: {}".format(phase, epoch, losses[0], losses[1], losses[2], losses[3]))
...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader(args.dataset+'bmnid')
    ...
    v_loader = data_loader(data_path, is_transform=True, split='valbmdswat3d', img_size=(args.img_rows, args.img_cols), img_norm=args.img_norm)
    ...
    # Setup Model
    model = get_model(args.arch, n_classes,in_channels=4)
    ...
    #Log file:
    experiment_name='dnetccnl_htan_swat3dbmdepthFixedvYvXvXYRanChess48_l1recon10.5_noaug_retraindepthFixedvYvXvXYRanl1' #network_activation(t=[0,1])_dataset_lossparams_augmentations_trainstart
...
#CUDA_VISIBLE_DEVICES=1 python trainS3dbmnoimg.py --arch dnetccnl --dataset swat3d --img_rows 128 --img_cols 128 --img_norm --n_epoch 250 --batch_size 50 --l_rate 0.0001