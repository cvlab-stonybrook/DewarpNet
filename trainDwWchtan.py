...
# Activation is hardtanh, this code has no batchnorm [doesn't train], and uses vanilla cropped unet as arch [and is hard to train]
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            if (i+1) % 50 == 0:
                ...
                if args.visdom:
                    ...
                    vis.images(out_arr,
                               win=train_out_win,
                               opts=dict(title='Train Outputs', caption='Wc_htannobnorm_noaug'))
                    vis.images(label_arr,
                               win=train_labels_win,
                               opts=dict(title='Train Labels', caption='Wc_htannobnorm_noaug'))
                    vis.images(inp_arr,
                               win=train_inputs_win,
                               opts=dict(title='Train Inputs', caption='Wc_htannobnorm_noaug'))
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)):
            ...
            if args.visdom:
                ...
                vis.images(out_arr,
                           win=val_out_win,
                           opts=dict(title='Val Outputs', caption='Wc_htannobnorm_noaug'))
                vis.images(label_arr,
                           win=val_labels_win,
                           opts=dict(title='Val Labels', caption='Wc_htannobnorm_noaug'))
                vis.images(inp_arr,
                           win=val_inputs_win,
                           opts=dict(title='Val Inputs', caption='Wc_htannobnorm_noaug'))
        ...
        if val_mse < best_val_mse:
            ...
            torch.save(state, "./checkpoints/{}_{}_{}_{}_{}_htanadamnobnorm_dewarpWc_scratch30-60knoaug_l1_best_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
        ...
        if (epoch+1) % 10 == 0:
            ...
            state = {'epoch': epoch+1,
                     'model_state': model.state_dict(),
                     'optimizer_state' : optimizer.state_dict(),}
            torch.save(state, "./checkpoints/{}_{}_{}_{}_{}_htanadamnobnorm_dewarpWc_scratch30-60knoaug_l1_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
