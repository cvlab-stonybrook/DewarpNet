# Densenet decoder encoder with intermediate fully connected layers but no dropout and final activation
...
class dnetns(nn.Module):
    ...
    def __init__(self, img_size=128, in_channels=1, out_channels=2, filters=32,fc_units=100):
        ...
        self.encoder=waspDenseEncoder128(nc=self.nc,ndf=self.nf,ndim=self.ndim)
    ...
    def forward(self, inputs):
        ...
        fc_out=self.fc_layers(encoded).unsqueeze(-1).unsqueeze(-1)
