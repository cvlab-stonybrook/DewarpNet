# code to train backward mapping regression from world coordinates using first layer as CoordConv
# models are in checkpoints-bm/ 
# Best mse is using L1 loss, also tried reconstruction ssim, reconstruction mse, and l1,ssim on predicted bm
# Reconstruction losses doesn't work well (probable things to try: higher res reconstruction)
# L1 best: dnet_dewarpnet_27_0.000613202166278_0.000209715812731_dewarpbmmeshplit_scratch_cc_l1_best_model.pkl
# After other losses: dnet_dewarpnet_31_0.000773750245571_0.000337579371373_dewarpbmmeshplit_retrain_cc_l1reconssim_best_model.pkl
...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader(args.dataset+'bmnicc')
    ...
    # Setup Model
    model = get_model(args.arch, n_classes,in_channels=5)
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            labels_nchw=labels.transpose(3, 2).transpose(2, 1)
            loss=(0.4*l1loss)+(0.3*rloss) + (0.3*ssim)
            # loss=l1loss
            ...
            if args.visdom:
                ...
                vis.heatmap(labely_arr,
                           ...
                           opts=dict(title='Train Label 1y', caption='In progress..'))
                labelopts=dict(title='Train Label', caption='Gt bmcc unwarp')
                outopts=dict(title='Train Out', caption='Pred. bmcc unwarp')
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)):
            ...
            if args.visdom:
                ...
                labelopts=dict(title='Val Label', caption='Gt bmcc unwarp')
                outopts=dict(title='Val Out', caption='Pred. bmcc unwarp')
        ...
        if val_mse < best_val_mse:
            ...
            torch.save(state, "./checkpoints-bm/{}_{}_{}_{}_{}_dewarpbmmeshplit_retrain_cc_l1reconssim_best_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
        ...
        if (epoch+1) % 10 == 0:
            ...
            state = {'epoch': epoch+1,
                     'model_state': model.state_dict(),
                     'optimizer_state' : optimizer.state_dict(),}
            torch.save(state, "./checkpoints-bm/{}_{}_{}_{}_{}_dewarpbmmeshsplit_retrain_cc_l1reconssim_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
