...
in_dir='/media/hilab/HiLabData/Sagnik/FoldedDocumentDataset/data/DewarpNet/benchmark/crop/'
...
def test(args):
    ...
    # batch.append(nocrop_img)
    # batch.append(real_nocrop)
    # batch.append(real_crop)
    # batch.append(real_nocrop2)
    # batch.append(real_nocrop3)
    # batch.append(real_crop3)
    # batch.append(real_nocrop4)
    # # print(batch[0].shape)
    # batch.append(real_nobg1)
    # batch.append(real_nobg2)

    for f in os.listdir(in_dir)[:30]:
        b=misc.imread(os.path.join(in_dir,f),mode='RGB')
        batch.append(b)
    ...
    if args.label:
        ...
        f, axarr = plt.subplots(6,batch_size/3)
    ...
    for i in range(batch_size):
        ...
        #for j in range(bs):
            #print imgs[j].shape
        axarr[j+1][i%10].imshow(pred)
        # axarr[1].imshow(disp_surf_norm_pred)
        axarr[j][i%10].imshow(inp)
        if args.label:
    ...
    plt.tight_layout()
    fname=args.model_path.split('/')[-1][:-4] + '.png'
    plt.savefig(os.path.join(args.out_path,fname), dpi=300)
    plt.show()
...
# CUDA_VISIBLE_DEVICES=1 python testBatch.py --model_path ./checkpoints-swat3d/unetnc_swat3d_97_0.000391505544732_0.000281962342521_htan_swat3dWcRGBvarYvarXvarXYRan_l1grad0.7bg_bghsaug_varYvarXvarXYRanl1grad0.7_best_model.pkl --dataset swat3d --img_norm --img_path /media/hilab/HiLabData/Sagnik/FoldedDocumentDataset/data/DewarpNet/benchmark/61_1copy.png
