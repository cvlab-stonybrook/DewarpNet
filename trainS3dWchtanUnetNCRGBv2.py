# code to train world coord regression from RGB Image (v2)
...
def write_log_file(experiment_name,losses, epoch, phase):
    with open('./checkpoints-swat3d/'+experiment_name+'.txt','a') as f:
        f.write("\n{} Epoch: {} Loss: {} MSE: {} BG: {} FG: {}".format(phase, epoch, losses[0], losses[1], losses[2], losses[3]))
...
def train(args):
    ...
    #Log file:
    experiment_name='htan_swat3dWcRGBvarY_scinv_bghsaug_retrainFixed' #activation_dataset_lossparams_augmentations_trainstart
    log_file=open('./checkpoints-swat3d/'+experiment_name+'.txt','w+')
    log_file.write('\n---------------  '+experiment_name+'  ---------------\n')
    log_file.close()

    best_val_mse = 99990.0
    log_count=0
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            loss =scinv(pred_nchw,labels_nchw)
            ...
            if (i+1) % 50 == 0:
                ...
                if args.visdom:
                    ...
                    vis.images(out_arr,
                               win=train_out_win,
                               opts=dict(title='Train Outputs', caption='Wc_bghsaug_scinv'))
                    vis.images(label_arr,
                               win=train_labels_win,
                               opts=dict(title='Train Labels', caption='Wc_bghsaug_scinv'))
                    vis.images(inp_arr,
                               win=train_inputs_win,
                               opts=dict(title='Train Inputs', caption='Wc_bghsaug_scinv'))

        print("Training L1:%4f" %(avgl1loss.item()/len(trainloader)))
        train_mse=train_mse/len(trainloader)
        print("Training MSE:'{}'".format(train_mse))
        train_losses=[avgl1loss.item()/len(trainloader), train_mse, avg_fg/len(trainloader),avg_bg/len(trainloader)]
        write_log_file(experiment_name, train_losses,epoch+1,'Train')
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)):
            ...
            if args.visdom:
                ...
                vis.images(out_arr,
                           win=val_out_win,
                           opts=dict(title='Val Outputs', caption='Wc_bghsaug_scinv'))
                vis.images(label_arr,
                           win=val_labels_win,
                           opts=dict(title='Val Labels', caption='Wc_bghsaug_scinv'))
                vis.images(inp_arr,
                           win=val_inputs_win,
                           opts=dict(title='Val Inputs', caption='Wc_bghsaug_scinv'))
        ...
        val_losses=[val_loss.item()/len(valloader), val_mse, val_fg/len(valloader),val_bg/len(valloader)]
        write_log_file(experiment_name, val_losses, epoch+1, 'Val')
...
#CUDA_VISIBLE_DEVICES=1 python trainS3dWchtanUnetNCRGB.py --arch unetnc --dataset swat3d --img_rows 256 --img_cols 256 --n_epoch 200 --batch_size 39 --l_rate 0.001 --img_norm --resume ./checkpoints-swat3d/unetnc_swat3d_141_0.00152933679055_0.00156929825059_htan_swat3dWcRGBvarY_l1_bgaug_retrainFixed_best_model.pkl --visdom
