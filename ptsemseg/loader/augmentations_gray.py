# augmentations code for grayscale input
# this code has errors at line 182
...
def cropimg(img,lbl,msk):
    ...
    if chance>0.7:
        ...
        # axarr[3].imshow(nmsk)
        # plt.show()
        nimg=cv2.resize(nimg,(img.shape[1],img.shape[0]),cv2.INTER_LANCZOS4)
        nlbl=cv2.resize(nlbl,(img.shape[1],img.shape[0]),cv2.INTER_LANCZOS4)
        nmsk=cv2.resize(nmsk,(img.shape[1],img.shape[0]),cv2.INTER_LANCZOS4)
        # print(nimg.shape)
        return nimg,nlbl,nmsk
    else:
        return img,lbl,msk
...
def augmentbg(root,im_name,img,lbl):
    ...
    if chance>0.1:
        ...
        with open(os.path.join(root,'augtexnames.txt'),'r') as f:
            for i in range(tex_id):
                txpth=f.readline().strip()

        # print(tex_root+txpth)
        if chance <= 0.5 :
            tex=cv2.imread(os.path.join(root,txpth)).astype(np.uint8)
            tex=cv2.resize(tex,(img.shape[1],img.shape[0]),cv2.INTER_LANCZOS4)
...
def call_augmentations(root,im_name,img,lbl):
    ...
    # nimg=change_hue_sat(img)
    nimg,nlbl,nmsk=augmentbg(root,im_name,img,lbl)
    # nimg=add_noise(nimg)
