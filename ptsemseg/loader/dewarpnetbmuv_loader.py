...
class dewarpnetbmuvLoader(data.Dataset):
    """
    Data loader for the  semantic segmentation dataset.
    """
    def __init__(self, root, split='trainWcdewarp', is_transform=False,
        ...
        for split in ['trainWcdewarp', 'valWcdewarp']:
    ...
    def __getitem__(self, index):
        ...
        wc_path = pjoin(self.root, 'wc',foldr,wc_name) 
        uv_name = 'DCX'+lbl_id+lbl_name[3]+'.exr'         #DCX20001.exr
        uv_path = pjoin(self.root, 'uv',foldr,uv_name)

        lbl_name = 'DCX'+lbl_id+lbl_name[3]+'.mat'         #DCX20001.mat
        lbl_path = pjoin(self.root, 'bm',foldr,lbl_name) 
        ...
        if self.is_transform:
            im, lbl = self.transform(im, wc,uv, bm)
        return im, lbl


    def transform(self, img, wc, uv, bm):
        ...
        uv = cv2.resize(uv, (self.img_size[0], self.img_size[1]), cv2.INTER_LANCZOS4)
        ...
        # print wc.shape
        # print 'here'
        img=np.concatenate([img,wc,uv],axis=0)
