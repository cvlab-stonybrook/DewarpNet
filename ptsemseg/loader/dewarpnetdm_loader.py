...
class dewarpnetdmLoader(data.Dataset):
    """
    ...
    def __init__(self, root, split='trainBmMeshsplitdm', is_transform=False,
        ...
        self.n_classes = 1                                      ##################<----------------- Change classes back to 3 when not mask
    ...
    def __getitem__(self, index):
        ...
        lbl_name = 'DMX'+lbl_id+lbl_name[3]+'.exr'         #WCX20001.exr
        lbl_path = pjoin(self.root, 'dmap-corrmesh',foldr,lbl_name)
        # print(lbl_path)
        ...
        lbl = cv2.imread(lbl_path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)[:,:,0]
...
#     dst = dewarpnetdmLoader(root=local_path, split='trainBmMeshsplitnew', is_transform=True, augmentations=True)
