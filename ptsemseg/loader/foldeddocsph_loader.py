...
class foldeddocsphLoader(data.Dataset):
    """
    Data loader for the  semantic segmentation dataset.
    """
    def __init__(self, root, split='trainSph', is_transform=False,
    ...
    def __getitem__(self, index):
        ...
        if 'train' in self.split:
            im_path = pjoin(self.root, 'ImagesSphCamTrain',  im_name + '.png')
            lbl_path = pjoin(self.root, 'depthMapExrsSphCamTrain','DepthMapEXR'+im_name[5:] + '.exr') #DepthMapEXR_10001.exr
        else:
            im_path = pjoin(self.root, 'ImagesSphCamTest',  im_name + '.png')
            lbl_path = pjoin(self.root, 'depthMapExrsSphCamTest','DepthMapEXR'+im_name[5:] + '.exr') #DepthMapEXR_10001.exr
        # print(im_path)
        im = m.imread(im_path)
...
#     dst = foldeddocsphLoader(root=local_path, split='trainSph', is_transform=True)
