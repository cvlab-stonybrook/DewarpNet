...
def vistensor(tensor, epoch, ch=0, allkernels=False, nrow=8, padding=1): 
    ...
    plt.savefig('./generated/filters_layer1_varcam_'+str(epoch)+'.png')
...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader(args.dataset+'sph')
    ...
    v_loader = data_loader(data_path, is_transform=True, split='valSph', img_size=(args.img_rows, args.img_cols), img_norm=args.img_norm)
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            labels_nchw=labels.unsqueeze(1)
            #print(labels_3.shape)
            target_flat=target_nhwc.view(-1,1)
            pred=softp(target)
            pred_flat=softp(target_flat)
            ...
            loss = loss_fn(pred_flat, labels.view(-1,1))+nlossl1
            # avgl1loss+=l1loss_
            # avg_nl1+=nlossl1
            avg_loss+=loss
            #loss = 0.5*nloss(pred, labels_nchw)
            train_mse+=MSE(pred_flat, labels.view(-1,1)).item()
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)):
            with torch.no_grad():
                ...
                gt = labels_val.cpu()
                #sdissim=1-ssim_loss(target_nhwc,labels_3)
                loss = loss_fn(pred, gt.view(-1,1))
                val_loss+=loss
                val_mse+=MSE(pred, gt.view(-1,1))
            if args.visdom:
                vis.line(
        ...
        if val_mse < best_val_mse:
            ...
            torch.save(state, "./checkpoints/{}_{}_{}_{}_{}_sp_sphcam_retrain_l1norml1_best_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
        ...
        if (epoch+1) % 10 == 0:
            ...
            state = {'epoch': epoch+1,
                     'model_state': model.state_dict(),
                     'optimizer_state' : optimizer.state_dict(),}
            torch.save(state, "./checkpoints/{}_{}_{}_{}_{}_sp_sphcam_retrain_l1norml1_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
