# code to train world coord regression from Grayscale Image
# models are saved in checkpoints/ 
# Activation is hardtanh, uses pix2pix unet as arch [this unet doesn't fail on real images]
# Loss is L1
# L1 best model (all augmentations): unetnc_dewarpnet_48_0.00164450611919_0.000434351101714_htan_dewarpWcgray_scratch30-60kallaug_l1_best_model.pkl
# L1 best model (no augmentations): unetnc_dewarpnet_57_0.00190896564163_0.000640385559255_htan_dewarpWcgray_scratch30-60knoaug_l1_best_model.pkl
# using jointloss: unetnc_dewarpnet_52_0.00185314670671_0.000808148162782_htan_dewarpWcgray_retrain30-60knoaug_l1jloss_best_model.pkl
...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader(args.dataset+'wcg')
    ...
    # Setup Model
    model = get_model(args.arch, n_classes,in_channels=1)
    ...
    for epoch in range(epoch_start,args.n_epoch):
            ...
            if (i+1) % 50 == 0:
                ...
                if args.visdom:
                    ...
                    for c in choices:
                        ...
                        inp_arr[idx,1,:,:]=inp_cpu[c][0,:,:]
                        inp_arr[idx,2,:,:]=inp_cpu[c][0,:,:]
                    ...
                    vis.images(out_arr,
                               win=train_out_win,
                               opts=dict(title='Train Outputs', caption='Wc_htanjloss_noaug'))
                    vis.images(label_arr,
                               win=train_labels_win,
                               opts=dict(title='Train Labels', caption='Wc_htanjloss_noaug'))
                    vis.images(inp_arr,
                               win=train_inputs_win,
                               opts=dict(title='Train Inputs', caption='Wc_htanjloss_noaug'))
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)):
            ...
            if args.visdom:
                ...
                vis.images(out_arr,
                           win=val_out_win,
                           opts=dict(title='Val Outputs', caption='Wc_htanjloss_noaug'))
                vis.images(label_arr,
                           win=val_labels_win,
                           opts=dict(title='Val Labels', caption='Wc_htanjloss_noaug'))
                vis.images(inp_arr,
                           win=val_inputs_win,
                           opts=dict(title='Val Inputs', caption='Wc_htanjloss_noaug'))
        ...
        if val_mse < best_val_mse:
            ...
            torch.save(state, "./checkpoints/{}_{}_{}_{}_{}_htan_dewarpWcgray_retrain30-60knoaug_l1jloss_best_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
        ...
        if (epoch+1) % 10 == 0:
            ...
            state = {'epoch': epoch+1,
                     'model_state': model.state_dict(),
                     'optimizer_state' : optimizer.state_dict(),}
            torch.save(state, "./checkpoints/{}_{}_{}_{}_{}_htan_dewarpWcgray_retrain30-60knoaug_l1jloss_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
