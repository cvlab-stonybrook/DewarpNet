# code to train backward mapping regression from depth map (no CoordConv)
# models are in checkpoints-bmdm/ 
# Best mse is using L1 loss, also tried reconstruction ssim, reconstruction mse, and l1,ssim on predicted bm
# L1 best: dnet_dewarpnet_20_0.000590694311541_0.000316077169267_dewarpdmap_scratch_l1_best_model.pkl
# After other losses: dnet_dewarpnet_80_0.000717222224921_0.000188794422155_dewarpdmap_scratch_ssiml1reconssim_model.pkl
import sys, os
import torch
import visdom
import argparse
...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader(args.dataset+'bmdm') # dewarpnetbmdmLoader
    ...
    best_val_mse =0.000590694311541
    log_count=0
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            if args.visdom:
                ...
                vis.heatmap(labely_arr,
                           ...
                           opts=dict(title='Train Label 1y', caption='In progress..'))
                labelopts=dict(title='Train Label', caption='Dmap Gt unwarp')
                outopts=dict(title='Train Out', caption='Dmap Pred. unwarp')
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)):
            ...
            if args.visdom:
                ...
                labelopts=dict(title='Val Label', caption='Dmap Gt unwarp')
                outopts=dict(title='Val Out', caption='Dmap Pred. unwarp')
        ...
        if val_mse < best_val_mse:
            ...
            torch.save(state, "./checkpoints-bmdm/{}_{}_{}_{}_{}_dewarpdmap_scratch_ssiml1reconssim_best_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
        ...
        if (epoch+1) % 10 == 0:
            ...
            state = {'epoch': epoch+1,
                     'model_state': model.state_dict(),
                     'optimizer_state' : optimizer.state_dict(),}
            torch.save(state, "./checkpoints-bmdm/{}_{}_{}_{}_{}_dewarpdmap_scratch_ssiml1reconssim_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
