...
def test(args,img_path,fname):
    ...
    # Predict WC
    # Setup Model
    wc_model = get_model(wc_model_name, wc_n_classes, in_channels=1)
    ...
    bminp_wc=cv2.resize(pred_wc,(bm_loader.img_size[0],bm_loader.img_size[1]),cv2.INTER_LANCZOS4)
    ...
    # concat image+wc
    bminp = np.concatenate([bminp_img,bminp_wc,xx_coord,yy_coord],axis=0)
    bminp = np.expand_dims(bminp, 0)

    bm_input = torch.from_numpy(bminp).float()
    # predict bm 
    bm_model = get_model(bm_model_name, bm_n_classes, in_channels=5)
