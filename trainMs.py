...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader(args.dataset+'ms')
    ...
    # print (n_classes)
    # Setup Model
    model = get_model(args.arch, n_classes,in_channels=3,is_batchnorm=True)
    ...
    for epoch in range(epoch_start,args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            pred_nhwc=sigm(target_nhwc)
            pred_nchw=sigm(target)
            # print(pred_nhwc.shape)
            ...
            if (i+1) % 50 == 0:
                ...
                if args.visdom:
                    ...
                    for c in choices:
                        ...
                        out_arr[idx,0,:,:]=target_cpu[c]
                        label_arr[idx,0,:,:]=labels_cpu[c]
                    ...
                    vis.images(out_arr,
                               win=train_out_win,
                               opts=dict(title='Train Outputs', caption='Ms_noaug'))
                    vis.images(label_arr,
                               win=train_labels_win,
                               opts=dict(title='Train Labels', caption='Ms_noaug'))
                    vis.images(inp_arr,
                               win=train_inputs_win,
                               opts=dict(title='Train Inputs', caption='Ms_noaug'))
        ...
        for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)):
            with torch.no_grad():
                ...
                pred=sigm(target_nhwc).data.cpu()
                pred_nchw=sigm(target).data.cpu()
            ...
            if args.visdom:
                ...
                vis.images(out_arr,
                           win=val_out_win,
                           opts=dict(title='Val Outputs', caption='Ms_noaug'))
                vis.images(label_arr,
                           win=val_labels_win,
                           opts=dict(title='Val Labels', caption='Ms_noaug'))
                vis.images(inp_arr,
                           win=val_inputs_win,
                           opts=dict(title='Val Inputs', caption='Ms_noaug'))
        ...
        if val_mse < best_val_mse:
            ...
            torch.save(state, "./checkpoints/{}_{}_{}_{}_{}_sigm_dewarpMs_scratch30-60knoaug_bce_best_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
        ...
        if (epoch+1) % 10 == 0:
            ...
            state = {'epoch': epoch+1,
                     'model_state': model.state_dict(),
                     'optimizer_state' : optimizer.state_dict(),}
            torch.save(state, "./checkpoints/{}_{}_{}_{}_{}_sigm_dewarpMs_scratch30-60knoaug_bce_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
