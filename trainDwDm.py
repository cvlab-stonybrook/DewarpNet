# code to regress depthmap using grayscale images as input 
# checkpoints are in checkpoints-dm
# Activation is hardtanh
# L1 loss: unetnc_dewarpnet_55_0.00216895551421_0.000753533210519_htan_dewarpDmap_scratch30-60knoaug_l1_best_model.pkl
# To try: surface normal loss, gradient loss
...
def train(args):
    ...
    # Setup Dataloader
    data_loader = get_loader(args.dataset+'dm')
    ...
    if args.visdom:
        ...
        train_inputs_win = vis.images(np.full((5,3, args.img_rows, args.img_cols),0),
                                   opts=dict(title='Train Inputs', caption='Dmap_htan_noaug'))
        train_labels_win = vis.images(np.full((5,3, args.img_rows, args.img_cols),0),
                                   opts=dict(title='Train labels', caption='Dmap_htan_noaug'))
        train_out_win = vis.images(np.full((5,3, args.img_rows, args.img_cols),0),
                                   opts=dict(title='Train Outputs', caption='Dmap_htan_noaug'))
        
        val_inputs_win = vis.images(np.full((5,3, args.img_rows, args.img_cols),0),
                                   opts=dict(title='Val Inputs', caption='Dmap_htan_noaug'))
        val_labels_win = vis.images(np.full((5,3, args.img_rows, args.img_cols),0),
                                   opts=dict(title='Val Labels', caption='Dmap_htan_noaug'))
        val_out_win = vis.images(np.full((5,3, args.img_rows, args.img_cols),0),
                                   opts=dict(title='Val Outputs', caption='Dmap_htan_noaug'))

    # Setup Model
    model = get_model(args.arch, n_classes,in_channels=1,is_batchnorm=False)
    ...
    for epoch in range(epoch_start,args.n_epoch):
            ...
            if (i+1) % 50 == 0:
                ...
                if args.visdom:
                    ...
                    for c in choices:
                        ...
                        out_arr[idx,1,:,:]=target_cpu[c]
                        label_arr[idx,1,:,:]=labels_cpu[c]
        ...
        if val_mse < best_val_mse:
            ...
            torch.save(state, "./checkpoints-dm/{}_{}_{}_{}_{}_htan_dewarpDmap_scratch30-60knoaug_l1_best_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
        ...
        if (epoch+1) % 10 == 0:
            ...
            state = {'epoch': epoch+1,
                     'model_state': model.state_dict(),
                     'optimizer_state' : optimizer.state_dict(),}
            torch.save(state, "./checkpoints-dm/{}_{}_{}_{}_{}_htan_dewarpDmap_scratch30-60knoaug_l1_model.pkl".format(args.arch, args.dataset, epoch+1,val_mse,train_mse))
